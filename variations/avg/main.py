...
def arima_forecast(dataset_series):
	# define the model
	model = ARIMA(dataset_series, order=(3,0,0)) #ARIMA:(Autoregressive Integrated Moving Average)
	# fit the model
...
def main():	
	...
	for name_id,dataset_id in dataset_group_id:
		...
		for i in range(len(filtered_dataset)):
			...
			#print(train)
			#break
			print("predicted: "+str(np.mean(predict_id))+"  actual:"+str(np.mean(tests)))
		#convert data again from logarithm form
			prediction.append(np.mean(np.exp(predict_id)))
			test.append(np.mean(tests))
