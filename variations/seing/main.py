...
def arima_forecast(dataset_series,dateset,start_index,end_index):
	...
	model = SARIMA(res, order=(3, 0, 0), seasonal_order=(3, 0, 0, 20),
                          simple_differencing=True) #ARIMA:(Autoregressive Integrated Moving Average)
	# fit the model
	model_fit = model.filter(model.start_params)
	# make forecast
	#print(model_fit.summary())
	forecast= model_fit.predict(start=len(dataset_series),end=len(dataset_series)+300)
	print(forecast)
...
def main():	
	...
	start_index = '2016-01-25'
	end_index = '2016-01-26'
	pred={}
